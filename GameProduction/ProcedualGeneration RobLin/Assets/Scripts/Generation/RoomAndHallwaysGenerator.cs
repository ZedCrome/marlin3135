...
public class RoomAndHallwaysGenerator : TileMapGeneratorTemplate
{
	...
	private void GenerateRoom(Vector3Int startPos, int roomWidth, int roomHeight)
	{
		...
		{
			float value = Mathf.PerlinNoise(i * 0.1f + noiseOffset.x, i * 0.1f + noiseOffset.y);
			int yOffset = Mathf.RoundToInt(value * 6) -4;
